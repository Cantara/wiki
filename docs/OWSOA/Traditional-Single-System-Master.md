...
# What
In a [Traditional Single System Master](Traditional<sub>~Single</sub>~System-Master.md) scenario you get data from one or many core systems, but one of the core systems is the master and is the only system where changes are saved. There is no meaning in fetching the "same" data from more than one core system because no synchronization will be done.
# When
# How
