...
Since one of Whydah's main design goals is ease of use and ease of integration, we have implemented a variation of ABAC. Like most ABAC implementations, we provide the application with several access-attributes for the user. Typically we see that the **RoleName** is used in combination with **SecurityLevel**, **ApplicationName** and properties in the **On-BehalfOf** attribute. 
...
\[1\](1.md) We can scale the load to Whydah, and even if this will increase the size of the UserTokens, wcan handle that. But having administrators manage this complexity in a system with tens or hundreds of applications does result in over-simplification (and less security) and lots of access-pains as a result or mis-intrepidation and lack of inner-knowledge of the specific application access properties.
