...
In JigZaw context, "_Divide and conquer_" is the concept of dividing a problem into sub-problems until we know how to solve them. This is a well-known technique for handling something complex and/or complicated. This principle is the cornerstone of the test strategy and is the origin for the name JigZaw. _Jigsav puzzles_ got their name from a painting divided into small pieces with a jigsaw. The same as we do to a complicated test problem; we split it into multiple smaller problems. 
...
JigZaw promotes the concept of using _tests to drive the development_, but the recommended process is more sophisticated than the [test-first](http://www.extremeprogramming.org/rules/testfirst.html) principle recommended by eXtreme Programming and TDD. The first part of David Heinemeier Hansson's blogpost [TDD is dead. Long live testing.](http://david.heinemeierhansson.com/2014/tdd-is-dead-long-live-testing.html) explains some of our objections to the test-first approach. 
