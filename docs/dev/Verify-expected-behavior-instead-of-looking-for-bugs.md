...
The difference may seem subtle, but there is a difference between **_the specification_** and **_works as expected_**. _Works as expected_ is not one, precise definition. Instead it refers to the different views we can have on a system and that each test should focus on a single aspect. Works as expected has thus a very strong focus on regression-testing.
