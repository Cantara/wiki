...
On rare occasions you have a number of standalone artifacts that have the same release cycle. If these projects are _always_ releases together it is possible to add them as modules to a multi<sub>~module Maven project. This approach differs from the builder</sub>~pom in three ways 
...
The difference between "_inheriting from a parent project, and being managed by an multi<sub>~module project_". The difference is explained in [Maven: The Definitive Guide - 9.6.2. Multi</sub><sub>module vs. Inheritance](http://www.sonatype.com/book/reference/pom</sub><sub>relationships.html#sect</sub><sub>multi</sub><sub>vs</sub>~inherit).
