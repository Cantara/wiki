...
#### Tactics 
This paragraph will explain how we can apply the [JigZaw Multidimensional Test Categorization](JigZaw-Multidimensional-Test-Categorization.md) to discuss and choose different types of tests. 
...
1. Basic integration with the JMS server and the Destination can be tested using an in-memory JMS server (e.g. ActiveMQ). If the JMS server is run locally and controlled by the test, it can be categorized as **with platform** and **out-of-process**. These types of tests are slower and more complex than the previous type. 
1. Full-blown testing (think clustering, fail-over, etc.) of the JMS integration requires an externally deployed JMS server. These types of tests can be categorized as **with platform** and **distributed** (not only out-of-process). These are the most expensive types of tests with regards to speed and complexity. 

**Example 2**: A service with a synchronous webservice endpoint 

1. The processing/generation of data should be possible to test using fast, **in-process** developer (unit) tests with no platform dependencies. 
1. Basic integration with the webservice can be tested using an in-memory application server (e.g. Jetty). If the server is run locally and controlled by the test, it can be categorized as **with platform** and **out-of-process**. These types of tests are slower and more complex than the previous type. 
1. Full-blown testing (think clustering, fail-over, etc.) of the webservice integration requires an externally deployed webservice. These types of tests can be categorized as **with platform** and **distributed** (not only out-of-process). These are the most expensive types of tests with regards to speed and complexity.
