...
The dimension-annotations signify a contract betweeen those who create the tests and those who run them. This forms a very powerful framework that makes it easier for the developer or the CI system to quickly drill down to an intended test suite, or widen the test scope to do full system testing when the need arises.

#### Implementation 

TestNG provides group functionality and run-time configuration to determine which groups to include in a test run. The Maven support is poor, but dimensions can be implemented with a combination of environment variables/command line parameters and Maven profiles. The Spring TestContext framework also supports some test grouping functionality.
...
Deciding when to run a certain group of tests depends on a number of factors, e.g., the capabilities of the CI-server. Important and/or complex functionality should receive high priority and will thus also effect the chosen strategy. With this disclaimer, we suggest the following baseline as a starting point for new projects: 
